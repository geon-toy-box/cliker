...
- 2026-06-23 · **버그픽스(출시 후) — 클릭 시 무음** (commit `5322e97`). 사용자 보고로 진단: logcat상 매 play가 미디어 오디오 포커스를 요청·즉시 상실(`onAudioFocusChange(-1)`)해 클릭이 끊김. `AudioPlayersBackend`에 SFX `AudioContext`(`AndroidAudioFocus.none` + sonification)를 `AudioPool.create`로 주입. 객관 확인: 포커스 요청 0(폭주 제거), analyze clean, 127 tests green. **가청 여부는 자동 QA 불가 → 사용자 확인 대기**(확인되면 릴리스 AAB 재빌드 필요). 미해결 시 다음 단계: PlayerMode.lowLatency→mediaPlayer.
